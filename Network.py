...
class ScaledDotProductAttention(nn.Module):
    ...
    # temperature here is not temperature but just the division using square root of d_k
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # this part will return both final score mapping output (Q*K*V) and the attention score (Q*K)
        return output, attn
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, n_head, d_k, d_v, dropout=0.1):
        ...
        self.d_k = d_k
        self.d_v = d_v
        # or just self.d_k = d_model//nhead, no need to pass d_k and d_v to the constructor
        
        # nn.Linear(n_feature_in, n_feature_out, bias=True)
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # Separate different heads: batch x len_q x n_head x d_k
        # .view() is used to reshape the tensor
        q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)
        k = self.w_ks(k).view(sz_b, len_k, n_head, d_k)
        v = self.w_vs(v).view(sz_b, len_v, n_head, d_v)

        # Transpose for attention dot product: batch x nhead x len_q x d_k
        # swap axes 1 and 2
        ...
        #print(attn.shape)
        # Transpose to move the head dimension back: batch x len_q x n x dv
...
class ConvTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src , mask):
        res = src
        # permute(12(all biophysical models), 3(seq, loop, struct), len_seq).permute(12(all biophysical models), len_seq, 3(seq, loop, struct))
        ...
        # position-wise feedforward network FFN with dropout
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        # residual dropout after deconvolution
...
# kmer_aggregation is not used
# kmers is equivalent to nlayers (k = kmers(or nlayers) + 1 - i)
class NucleicTransformer(nn.Module):
    ...
    def __init__(self, ntoken, nclass, ninp, nhead, nhid, nlayers, kmer_aggregation, kmers, stride=1,dropout=0.5,pretrain=False,return_aw=False):
        ...
        self.encoder = nn.Embedding(ntoken, ninp) # (12, len(seq), 3, ninp)
        ...
        self.decoder = Decoder(ninp,ninp//2,nclass)
    ...
    def forward(self, src, mask=None):
        B,L,_=src.shape # (batch_size, len(seq), 3)
        src = src
