...
#checkpointing
checkpoints_folder='checkpoints/checkpoints_fold{}'.format((opts.fold))
csv_file='log_fold/log_fold{}.csv'.format((opts.fold))
...
long_dataset=RNADataset(long_data.sequence.to_list(),np.zeros(len(ls_indices)),ids, np.arange(len(ls_indices)),curr_path,training=False)
...
short_dataset=RNADataset(short_data.sequence.to_list(),np.zeros(len(ss_indices)), ids, np.arange(len(ss_indices)),curr_path,training=False)
...
# The ascending order of the augmented bppm provided by the author.
...
submission=pd.read_csv(os.path.join(curr_path,'sample_submission.csv'))
...
for i, pkg in enumerate(avail_packages):
    ...
    pkg_sub.to_csv(f"data/{pkg}.csv",index=False)
