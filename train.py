import os
# trying to fix the error forrtl: error(200): program aborting due to control-C event
...
def train_fold():
    ...
    # filter by using signal_to_noise
    ...
    # error adjustedted loss from the Nucleic Transformer paper (He et al., 2021)
    error_weights=opts.error_alpha+np.exp(-error_weights*opts.error_beta) # Yet to be tested
    ...
    dataset=RNADataset(train_seqs,train_labels,train_ids, train_ew, curr_path)
    #val_dataset=RNADataset(val_seqs,val_labels, val_ids, val_ew, opts.path, training=False)
    val_dataset=RNADataset(val_seqs,val_labels, val_ids, val_ew, curr_path, training=False)
    ...
    # EXTRA: Options to load pretrained model
    ...
    # optimizer=Ranger21(model.parameters(), weight_decay=opts.weight_decay, lr=0.001,
    #                    num_epochs=opts.epochs, num_batches_per_epoch=len(dataloader),
    #                    use_madgrad=True)
    ...
    for epoch in range(opts.epochs):
        ...
        ground_truths=[]
        step=0
        # dataloader -> (batch_size, 12, 3, len(seq))
        ...
        #for step in range(1):
            ...
            lr=get_lr(optimizer)
            #print(lr)
            src=data['data'].to(device) # (12, 3, len(seq))
            labels=data['labels']
    ...
    get_best_weights_from_fold(opts.fold,top=3)
